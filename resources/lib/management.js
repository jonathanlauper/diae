(function () {
  ...
  function init() {
    ...
    $('#c-params-editor').bind('axel-save-done', function() {
      ...
      $('a[href="#c-pane-params"]').click(); // reloads pane content
        ...
        // contractors editing modal
    $('#c-pane-contractors').bind('coaching-update-contractors', openContractorsEditor);
    $('#c-contractors-editor').bind('axel-cancel-edit', function() { $('#c-contractors-editor-modal').modal('hide'); });
    $('#c-contractors-editor').bind('axel-save-done', function() {
      $('#c-contractors-editor-modal').modal('hide');
      $('a[href="#c-pane-contractors"]').click(); // reloads pane content
  ...
  function openParamsEditor (ev) {
    ...
    // Opens up modal window, loads it pre-defined template, loads target data inside it
  function openContractorsEditor (ev) {
    var src, wrapper, ed;
    wrapper = $axel('#c-contractors-editor');
    ed = $axel.command.getEditor('c-contractors-editor');
    ed.attr('data-src', null); // a. remove any potential data source previously set in b
    ed.ready = false; // forces template reload and transformation FIXME: change API
    ed.transform();
    if (wrapper.transformed()) {
      ed.attr('data-src', 'management/contractors'); // b. sets same URL as template as data source for 'save' command
      $('#c-contractors-editor-modal').modal('show');
    }
  }
  ...
  function showItemDetails (ev) {
    ...
    if (src) {
      ...
      if (src) {
        ...
        src = target.attr('data-contractors');
        if (src) {
          key = 'contractors';
        } 
        else {
  ...
  function itemSaveDone (ev, editor, source, xhr) {
    ...
    if (key === 'login') {
      ...
      reportContractorsUpdate(xhr.responseXML);
    }
  ...
  function reportProfileUpdate(response) {
    ...
    function reportContractorsUpdate(response) {
    var id = $('Value', response).text(),
        label = $('Name', response).text();
    $("span[data-contractors$='/" + id + "']").text(label);
  }
  ...
  function reportRemoteUpdate(response) {
    ...
    $('span[data-remote$=' + '"' + id + '"]').text(label).attr('data-remote', 'contractors/' + encodeURIComponent(contact).replace(/\./g, "%2E"));
