...
declare function local:gen-success( $id as xs:string, $ext as xs:string , $n as node()*) as element() {
  let
    $full-path := 'images/',
  ...
  return
    <html>
      <body>
        <script type='text/javascript'>window.parent.finishTransmission(1, "{$full-path}{$id}.{$ext}.{$n}")</script>
     </body>
    </html>
};
...
(: ======================================================================
   ...
   ======================================================================
:)
declare function local:uploadImage(
...
{
  let
    $actual-uri := concat($col-uri,"/", $trail),
    $in := <Image><Ref>{$fname}</Ref></Image>,
    $perms := 'rwxrwxr--',
    $isLogo := request:get-parameter('g', ()) = 'logo',
    $id := string($cur-index),
    $image-id := if ($isLogo) then concat($id, '-logo') else $id,
    $filename := replace(replace($fname, '\s+', '_-'),':','-'),
    ...
    $doc-uri := concat($col-uri, '/collections.xml')
  return
    if (($filtered[2] instance of xs:base64Binary) and (xdb:store( $actual-uri, $filename, $filtered[2], $mime-type))) then
      (
      compat:set-owner-group-permissions(concat( $actual-uri, '/', $filename), $user, $group, $perms),
      
      update replace $cur-index with attribute LastIndex { number($id) +1 },
      
      
      update insert $in into doc($doc-uri)//Collection[Name eq $trail]/Images,
      
      (:update insert $in into doc($doc-uri)//Name[. eq $trail],:)
      
      (:doc($doc-uri)//Images[.. eq ],:)
      ...
      local:gen-success($col-uri, $ext, doc($doc-uri)//Collection[Name eq $trail]/Images) (:doc(concat($col-uri, '/index.xml'))/Gallery/@LastIndex:)
      )[last()]
    else
...
(: ======================================================================
   Update image collections when new image is added
   ======================================================================
:)
declare function local:updateCollections(  
    $col-uri as xs:string,
    $user as xs:string,
    $group as xs:string,
    $filename as xs:string ) as element()*
{
let  $perms := 'rwxrwxr--'
return 
      local:gen-error("Erreur lors de la sauvegarde de l'image, réessayez avec une autre")
...
:)
declare function local:unzip($zip-filename as xs:string, $zip-data as xs:base64Binary){
    let $entry-filter := util:function(QName("local", "local:unzip-entry-filter"), 3)
    let $entry-filter-params := ()
    let $entry-data := util:function(QName("local", "local:unzip-entry-data"), 4)
    let $entry-data-params := 
        (
        <param name="zip-filename" value="{$zip-filename}"/>
        )

    let $sent := local:forwardZipToServer($zip-filename, $zip-data) (:TODO:remove comments:)
    let $unzip := compression:unzip($zip-data, $entry-filter, $entry-filter-params, $entry-data, $entry-data-params)
    return 
        <results>{$unzip}</results>
};

declare function local:unzip-entry-data($path as xs:string, $data-type as xs:string, $data as item()?, $param as item()*) {
    let $user := xdb:get-current-user()
    let $agp := request:get-attribute('xquery.group')
    let $group := if ($agp) then $agp else 'site-admin'
    let $cmd := oppidum:get-command()
    let $testor :=  $param[@name="zip-filename"]/@value/string()
    return
        if (not($data instance of xs:base64Binary))
        then local:gen-error('Le fichier téléchargéeee est invalideeee')
        else
          (: check photo binary stream has compatible MIME-TYPE :)
          let
            $ext:= local:get-extension($path),
            $mime-check := local:check-extension($ext)
          return
            if ( $mime-check != 'ok' )
            then local:gen-error($mime-check)
            else
              (: create image collection if it does not exist yet TODO: add collection to collections.xml:) 
                    ...
                    return
                      if (not($cur-index castable as xs:integer))
                        then local:gen-error("Erreur sur le serveur: impossible de générer un nom pour stocker l'image")
                                ...
                                local:uploadImage($col-uri, 'testor', $user, $group, $path, $cur-index ,$data, $ext))
};

declare function  local:unzip-entry-filter($path as xs:string, $data-type as xs:string, $param as item()*) as xs:boolean {
    if(not($data-type = ('folder','resource'))) then false()
    else
        true()
};

declare function local:forwardZipToServer($zip-filename as xs:string, $zip-data as xs:base64Binary){
    let $url := 'https://homedav.unifr.ch/LauperJo/imageServer/'
    let $request-headers := (<headers><header name="Authorization" value="Basic TGF1cGVySm86UTM4OTc1YTkyej8="/></headers>)(:Add authorization to server here "" : "Basic TGF1cGVySm86UTM4OTc1YTkyej8=" :)
    return
        http:send-request(<http:request method='MkCoL'>    
                                <http:header name="Authorization" value="Basic TGF1cGVySm86UTM4OTc1YTkyej8="/>
                           </http:request>, concat($url, $zip-filename))
    

};
(:
declare function local:forwardZipToServer($zip-data as xs:base64Binary){
    let $url := 'https://homedav.unifr.ch/LauperJo/imageServer/'
    let $request-headers := (<headers><header name="Authorization" value="Basic TGF1cGVySm86UTM4OTc1YTkyej8="/></headers>)(:Add authorization to server here "" : "Basic TGF1cGVySm86UTM4OTc1YTkyej8=" :)
    return
        httpclient:post($url, $zip-data, false(), $request-headers)
};
:)
(:
...
let $user := xdb:get-current-user()
let $cmd := oppidum:get-command()
let $split := tokenize($cmd/@trail, '/')
return
  let $data := request:get-uploaded-file-data('zip-file')
  return
    if (not($data instance of xs:base64Binary)) then local:gen-error('Le fichier téléchargé est invalideaaa')
    else
      let  $filename := request:get-uploaded-file-name('zip-file')
      return
          local:unzip($filename, $data)
...
return
  ...
  return
    if (not($data instance of xs:base64Binary))
    then local:gen-error('Le fichier téléchargéeee est invalide')
    else
      (: check photo binary stream has compatible MIME-TYPE :)
      let
                ...
                return
                  if (not($cur-index castable as xs:integer))
                    then local:gen-error("Erreur sur le serveur: impossible de générer un nom pour stocker l'image")
                            ...
                            local:do-upload($col-uri, $split[3] , $user, $group, $filename, $cur-index, $data, $ext))
:)