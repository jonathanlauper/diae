...
import module namespace search = "http://oppidoc.com/ns/xcm/search" at "../../../xcm/modules/persons/search.xqm";
...
declare function local:update-person( $person as element(), $submitted as element(), $lang as xs:string ) as element() {
  ...
  let $updated := template:do-update-resource('person', $id, $person, (), $submitted, $lang)
  return
    if (local-name($updated) eq 'success') then
      ajax:report-success('ACTION-UPDATE-SUCCESS', (), 
        if (request:get-parameter('next', ()) eq 'autofill') then
          ...
          return search:gen-person-sample($fresh-person, (), true(), $lang)
      )
    else
      $updated
};
...
declare function local:gen-person( $person as element(), $goal as xs:string, $lang as xs:string ) as element()* {
  if ($goal = 'read') then
    ...
    let $payload := template:gen-citation('person', $person/Id, $person/Information, $lang)
