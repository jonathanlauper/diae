...
declare variable $search:col-uri := '/db/sites/diae/pages/images/collections.xml';


declare function search:fetch-algorithm( $request as element() , $lang as xs:string ) as element()* {
  if ((count($request/*/*) + count($request/*[local-name(.)][normalize-space(.) != ''])) = 0) then (: empty request :)
    ...
    <Results>{ search:find-collection-results($lang) }</Results>
  else
    <Results>{ search:find-collection-results($request, $lang) }</Results>
};
declare function search:find-algorithm( $request as element(), $lang as xs:string ) as element()* {
    let $algorithms := globals:collection('global-info-uri')//Description[@Lang = $lang]//Selector[@Name eq 'Algorithms']
    let $collection := $request//Algorithm
    return
};
...
declare function search:get-default-request () as element() {
  ...
  return
    if ($profile/SearchCollectionRequest) then
      $profile/SearchCollectionRequest
    else
      <Request/>
};

(: ======================================================================
   Returns Collections matching request
   also returns individual Coach modal windows
   TODO: return Enterprise modal windows
   ======================================================================
:)
declare function search:fetch-collection-results ( $request as element() , $lang as xs:string ) as element()* {
  if ((count($request/*/*) + count($request/*[local-name(.)][normalize-space(.) != ''])) = 0) then (: empty request :)
    if (request:get-parameter('_confirmed', '0') = '0') then
      (
      <Confirm/>,
...
(: ======================================================================
   ...
   ====================================================================== 
:)
declare function search:find-collection-results ( $request as element(), $lang as xs:string ) as element()* {
  ...
  let $filter-enterprise := not(empty(($collection)))
...
(: ======================================================================
   ...
   ====================================================================== 
:)
declare function search:find-collection-results ( $lang as xs:string ) as element() {
  (: --- access control layer --- :)
  let $profile := user:get-user-profile()
  let $omni-sight := access:check-omniscient-user($profile)
  let $person := $profile/ancestor::Person  
...
declare function local:gen-result-sample ( $lang as xs:string ) as element()? {
  let $sample := ()
  return <Result><Column>Edit /modules/collection/search.[xqm,.xsl] to generate search results</Column></Result>
};
...
declare function local:gen-activity-sample ( $a as element(), $lang as xs:string ) as element() {
  ...
  return
    <Activity>
      { $a/@legacy }
      { $a/No }
      <Title>{ custom:gen-short-activity-title($a/ancestor::Case, $a, $lang) }</Title>
      <Coach>
        {
        if ($coach-id) then (
          <Id>{ $coach-id }</Id>,
...
(: ======================================================================
   Generates Open attribute with value 'y' which controls the generation
   of the case/activity title with a link or not to open it.
   Does some filtering to not generate it if already opened at the global
   scope (assumed hierarchy : Cases > Case > Activity)
   ======================================================================
:)
